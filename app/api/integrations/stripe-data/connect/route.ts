...
import { validateStripeKey, fetchStripePayments } from "@/lib/integrations/stripe-data"
...
export async function POST(request: Request) {
  ...
  const { workspace_id, secret_key, dataset_name } = await request.json()
  if (!workspace_id || !secret_key) return NextResponse.json({ error: "Missing fields" }, { status: 400 })
  ...
  const name = dataset_name || "Stripe Payments"

  try { await validateStripeKey(secret_key) }
  ...
  const { data: integration, error: intErr } = await adminClient.from("integrations").insert({
    workspace_id, provider: "stripe_data", name, status: "pending",
  ...
  try {
    const data = await fetchStripePayments({ secretKey: secret_key, datasetName: name })
